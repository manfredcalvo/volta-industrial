...
| 2 | App inference table — calls routed through the gateway, the budget block, and the guardrail blocking the all-data read | `app_inference_table.json` | ✅ routed calls + **7× HTTP 429 budget blocks** (token cap crossed) from the inference table; the guardrail block (`block-bulk-data-exfiltration` service policy) shown as a live gateway call (Delta guardrail rows ingest on the table's batch schedule) |
